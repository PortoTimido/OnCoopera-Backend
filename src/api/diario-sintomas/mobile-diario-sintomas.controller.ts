...
import {
  listDiarioQuerySchema,
  saveDiarioSchema,
  type ListDiarioQuery,
  type SaveDiarioRequestBody,
} from './diario-sintomas.schemas.js';
import {
  ErrorSwaggerResponseDto,
  PaginatedRegistrosDiariosSwaggerDto,
  RegistroDiarioSwaggerDto,
} from './diario-sintomas.swagger.js';
...
const voiceNoteInterceptor = FileInterceptor('notaVoz', {
  limits: { fileSize: MAX_AUDIO_SIZE, files: 1 },
  fileFilter: (_request, file, callback) =>
    callback(
      ALLOWED_AUDIO_TYPES.includes(file.mimetype)
        ? null
        : new BadRequestException('Formato de áudio inválido.'),
      ALLOWED_AUDIO_TYPES.includes(file.mimetype),
    ),
});
...
export class MobileDiarioSintomasController {
  ...
  list(
    ...
    @Query(new ZodValidationPipe(listDiarioQuerySchema)) query: ListDiarioQuery,
  ) {
    return this.listRegistros.execute(
      patientId(request),
      query.page,
      query.pageSize,
    );
  ...
  getToday(@Req() request: AuthenticatedRequest) {
    return this.getHoje.execute(patientId(request));
  }
  ...
  async getById(
    ...
    try {
      return await this.getRegistro.execute(patientId(request), id);
    } catch (error) {
      throw mapDiarioSintomasError(error);
    }
  }
  ...
  @ApiBody({
    schema: {
      type: 'object',
      required: ['humor', 'sintomas'],
      properties: {
        humor: {
          type: 'string',
          enum: ['MUITO_BEM', 'BEM', 'NEUTRO', 'MAL', 'MUITO_MAL'],
        },
        sintomas: { type: 'string', description: 'JSON de sintomas.' },
        notaVoz: { type: 'string', format: 'binary' },
        removerNotaVoz: { type: 'boolean' },
      },
    },
  })
  @ApiOkResponse({ type: RegistroDiarioSwaggerDto })
  ...
  async save(
    @Req() request: AuthenticatedRequest,
    @Body(new ZodValidationPipe(saveDiarioSchema)) body: SaveDiarioRequestBody,
    @UploadedFile() file?: UploadedVoiceNote,
  ) {
    try {
      return await this.saveHoje.execute(patientId(request), {
        ...body,
        notaVoz:
          file === undefined
            ? undefined
  ...
  async update(
    ...
    try {
      return await this.updateHoje.execute(patientId(request), id, {
        ...body,
        notaVoz:
          file === undefined
            ? undefined
  ...
  async voiceNote(
    ...
    try {
      ...
      if (registro.notaVozUrl === null)
        throw new BadRequestException('Registro não possui nota de voz.');
...
function patientId(request: AuthenticatedRequest): string {
  if (request.auth === undefined)
    throw new BadRequestException('Contexto autenticado ausente.');

...
export interface EnderecoApoio {
  id?: string;
  cep: string;
  logradouro: string;
  numero: string;
...
export function validateHorarios(horarios: HorarioApoio[]): void {
  ...
  for (const horario of horarios) {
    if (
      !Number.isInteger(horario.diaSemana) ||
      horario.diaSemana < 0 ||
      horario.diaSemana > 6
    )
      throw new DomainValidationError('Dia da semana invalido.');
    if (
      !/^\d{2}:\d{2}$/.test(horario.horarioInicio) ||
      !/^\d{2}:\d{2}$/.test(horario.horarioFim) ||
      horario.horarioInicio >= horario.horarioFim
    )
      throw new DomainValidationError('Intervalo de horario invalido.');
    byDay.set(horario.diaSemana, [
      ...(byDay.get(horario.diaSemana) ?? []),
      horario,
    ]);
  }
